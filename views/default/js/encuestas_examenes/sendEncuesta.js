...
elgg.encuestas_examenes.init_sendEncuesta = function() {
	var elgg = require('elgg');
	
	$encuesta_guid = $('input[name=guid]').val();
			...
			if($rest == 0){
				...
				if($('#noDuration').val() == '0'){
					if($('#automaticSend').val() == 'YES'){
   					...
   					elgg.forward('encuestas_examenes/all?tab=encuesta');
				}
			}else{
	...
	$(".elgg-form-encuestas-examenes-vote").on('submit', function(e){
		$empty = 0;
		$required = false;
		for($i=1;$i<=$('#numQEncuesta').val();$i++){
			$nomQ = 'Pregunta'+$i;
		...
		if($('#emptyResults').val() != 'NO' && $empty == $('#numQEncuesta').val() && $('.timeLeft').width() != 0){
			//alert($('.timeLeft').width());
			e.preventDefault();
			elgg.register_error(elgg.echo('encuestas_examenes:poll_empty'));
		}
		if($required && $empty){
			e.preventDefault();
...
//register init hook
elgg.register_hook_handler('init', 'system', elgg.encuestas_examenes.init_sendEncuesta);
